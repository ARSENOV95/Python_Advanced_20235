...
def create_ui(root, rows, cols, slots):
    ...
    labels = [[tk.Label(root, text=" ", width=4, height=2, bg="white", relief="solid") for _ in range(cols)]
              for _ in range(rows)]
    for r in range(rows):
        for c in range(cols):
    ...
    tk.Button(root, text = 'Reset',command=lambda: reset_game(matrix,rows,cols,labels)).grid(row=rows+1,column=cols//2)
